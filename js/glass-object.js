// Vanilla-JS port of CanvasUI's glass-object component (canvasui.dev),
// stripped of its React/Next wrapper since this site has no build step.
// Extrudes an SVG/image/GLB asset into a physically-based glass mesh lit
// by a baked studio environment. See vendor/three/LICENSE for Three.js's
// own MIT license.
...
// GlassObjectOptions:
//   src                   URL of the asset: GLB/glTF, SVG, PNG, JPEG, WebP, or GIF (sniffed from bytes, not the extension)
//   ior                   Index of refraction of the glass (1 to 2.33)
//   thickness             Thickness of the glass volume in scene units
//   roughness             Surface roughness (0 to 1); higher frosts the glass
//   dispersion            Chromatic dispersion (0 to 2) — rainbow fringing
//   clearcoat             Clearcoat layer on top of the glass (0 to 1)
//   tint                  CSS color tinting the glass volume; empty keeps it clear
//   tintDensity           How strongly the tint absorbs light through the volume
//   depth                 Extrusion depth of 2D assets, as a fraction of their longest side
//   bevel                 Edge rounding of extruded 2D assets (0 to 1)
//   highlight             Accent color of the studio ring light
//   environmentIntensity  Brightness of the studio environment lighting
//   background            CSS color behind the glass; empty keeps the canvas transparent
//   backgroundImage       Image shown as a backdrop behind the glass, refracted through it
//   scale                 Size of the asset's longest side in scene units
//   xOffset / yOffset     Offset in scene units
//   floatIntensity        Strength of the floating bob animation (0 disables)
//   rotationIntensity     Strength of the idle rocking rotation (0 disables)
//   floatSpeed            Speed of the float/rock animation
//   orbit / zoom          Let the user orbit/zoom the camera
//   autoRotate            Spin the camera turntable-style
//   autoRotateSpeed       Turntable speed when autoRotate is on
//   fov / cameraDistance  Camera field of view (degrees) / distance from the asset's center
//   dracoDecoderPath      Base URL of the Draco decoder, fetched only if a model needs it
//   onLoad / onError      Asset lifecycle callbacks
    ...
    for (let cy = 0; cy <= h; cy++) {
        for (let cx = 0; cx <= w; cx++) {
            const code =
                ...
                at(cx - 1, cy);
            switch (code) {
                case 1: segments.push([L(cx, cy), B(cx, cy)]); break;
                case 2: segments.push([B(cx, cy), R(cx, cy)]); break;
                case 3: segments.push([L(cx, cy), R(cx, cy)]); break;
                case 4: segments.push([T(cx, cy), R(cx, cy)]); break;
                case 5:
                    segments.push([L(cx, cy), T(cx, cy)]);
                    segments.push([B(cx, cy), R(cx, cy)]);
                    break;
                case 6: segments.push([T(cx, cy), B(cx, cy)]); break;
                case 7: segments.push([L(cx, cy), T(cx, cy)]); break;
                case 8: segments.push([L(cx, cy), T(cx, cy)]); break;
                case 9: segments.push([T(cx, cy), B(cx, cy)]); break;
                case 10:
                    segments.push([T(cx, cy), R(cx, cy)]);
                    segments.push([L(cx, cy), B(cx, cy)]);
                    break;
                case 11: segments.push([T(cx, cy), R(cx, cy)]); break;
                case 12: segments.push([L(cx, cy), R(cx, cy)]); break;
                case 13: segments.push([B(cx, cy), R(cx, cy)]); break;
                case 14: segments.push([L(cx, cy), B(cx, cy)]); break;
            }
        }
    }
...
function shapesFromImage(data) {
    ...
    for (let i = 0; i < loops.length; i++) {
        ...
        shapes.push(shape);
        owners.push({ loop: loops[i], area: Math.abs(signedArea(loops[i])), shape });
    }
    for (let i = 0; i < loops.length; i++) {
        if (depths[i] % 2 === 0) continue;
        const [x, y] = loops[i][0];
